<?php 
	...
	if(isset($_POST['submit']))
	{
		...
		if(in_array(strtolower($_POST['user']), $userArray) && in_array(strtolower($_POST['password']), $pwArray))
		{
			...
			{
				$access = 0;
				if(strtolower($_POST['user']) == 'beavis')
				{
					$sql = 'update password set logins = (logins + 1) where id = 1';
					$access = 'Access Granted<br><br>Logins: '.$login[1];
				}					
				if(strtolower($_POST['user']) == 'butthead') 
				{
					$sql = 'update password set logins = (logins + 1) where id = 2';
					$access = 'Access Granted<br><br>Logins: '.$login[2];
				}	
				if(strtolower($_POST['user']) == 'dana')
				{
					$sql = 'update password set logins = (logins + 1) where id = 3';
					$access = 'Access Granted<br><br>Logins: '.$login[3];
				}	
				if(strtolower($_POST['user']) == 'fox')
				{
					$sql = 'update password set logins = (logins + 1) where id = 4';
					$access = 'Access Granted<br><br>Logins: '.$login[4];
				}	
